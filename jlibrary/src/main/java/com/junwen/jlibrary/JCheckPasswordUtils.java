...
/**
 * 描述：检查密码强度 主要是 getPasswordLevel();方法
 * 作者：卜俊文
 * 创建：2016/9/19 16:53
 * 邮箱：344176791@qq.com
 */
public class JCheckPasswordUtils {
    ...
    public static int checkPasswordStrength(String passwd) {
        if (JStringUtils.equalsNull(passwd)) {
        ...
        if (JStringUtils.isNumeric(passwd) && ("01234567890".indexOf(passwd) > 0 || "09876543210".indexOf(passwd) > 0)) {
            level--;
        }
        ...
        if (len % 2 == 0) { // aaabbb
            ...
            if (JStringUtils.isCharEqual(part1) && JStringUtils.isCharEqual(part2)) {
                level--;
            }
        }
        if (len % 3 == 0) { // ababab
        ...
        if (JStringUtils.isNumeric(passwd) && len >= 6) { // 19881010 or 881010
            int year = 0;
            if (len == 8 || len == 6) {
                year = Integer.parseInt(passwd.substring(0, len - 4));
            }
            int size = JStringUtils.sizeOfInt(year);
        ...
        if (JStringUtils.isCharEqual(passwd)) {
            level = 0;
        }
     ...
     * 4-6 : midium;中等
     * 7-9 : strong;高
     * 10-12 : very strong; 非常高
     * >12 : extremely strong.极高
     * 作者:卜俊文
     * 邮箱:344176791@qq.com
     * 创建时间: 2016/9/19 17:20
     */
